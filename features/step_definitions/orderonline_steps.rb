...
When(/^I click the Tracker tab$/) do
  find(:xpath, '/html/body/header/nav/div[3]/ul/li[10]/a').click
end

Then(/^I should see Tracker page$/) do
  expect(page).to have_selector(:id, 'trackerPage')
end

When(/^I click the Menu tab$/) do
  begin
    if expect(page).to have_selector(:id, 'homePage')
      click('Menu')
    end
  rescue
...
Then(/^I should see Dominos Menu$/) do
  if expect(page).to have_selector(:id, 'genericMenuPage')
